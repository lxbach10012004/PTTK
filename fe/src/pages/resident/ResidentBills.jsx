...
const formatPaymentStatus = (status) => {
    switch (status) {
        case 'Chưa_thanh_toán': return <span className="px-2 py-1 inline-flex text-xs leading-5 font-semibold rounded-full bg-red-100 text-red-800">Chưa thanh toán</span>;
        case 'Đã_thanh_toán': return <span className="px-2 py-1 inline-flex text-xs leading-5 font-semibold rounded-full bg-green-100 text-green-800">Đã thanh toán</span>;
        case 'Quá_hạn': return <span className="px-2 py-1 inline-flex text-xs leading-5 font-semibold rounded-full bg-yellow-100 text-yellow-800">Quá hạn</span>;
        default: return <span className="px-2 py-1 inline-flex text-xs leading-5 font-semibold rounded-full bg-gray-100 text-gray-800">{status}</span>;
    }
};
...
function ResidentBills() {
  ...
  const filteredBills = bills.filter(bill => {
    ...
    const searchMatch = searchTerm.trim() === '' || 
      (bill.so_hoa_don && bill.so_hoa_don.toLowerCase().includes(searchTerm.toLowerCase())) ||
  ...
  const totalAmount = filteredBills.reduce((sum, bill) => {
    const amount = typeof bill.tong_tien === 'string' ? parseFloat(bill.tong_tien) : (bill.tong_tien || 0);
    return sum + (isNaN(amount) ? 0 : amount);
  }, 0);

  const unpaidAmount = filteredBills
    .filter(bill => bill.trang_thai === 'Chưa_thanh_toán' || bill.trang_thai === 'Quá_hạn')
    .reduce((sum, bill) => {
      const amount = typeof bill.tong_tien === 'string' ? parseFloat(bill.tong_tien) : (bill.tong_tien || 0);
      return sum + (isNaN(amount) ? 0 : amount);
    }, 0);
  ...
  const billTypes = [...new Set(bills.map(bill => bill.loai_hoa_don))].filter(Boolean);
  ...
  return (
    <div className="bg-white rounded-lg shadow-md">
      <div className="border-b border-gray-200 px-6 py-4">
        <h1 className="text-xl font-semibold text-gray-800">Danh sách Hóa đơn</h1>
        <p className="text-gray-600 text-sm mt-1">Quản lý và theo dõi các hóa đơn của bạn</p>
      </div>
      ...
      <div className="px-6 py-4 border-b border-gray-200">
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
          <div className="bg-blue-50 rounded-lg p-4">
            ...
            <div className="text-2xl font-semibold text-blue-700">{filteredBills.length}</div>
          </div>
          
          <div className="bg-green-50 rounded-lg p-4">
            <div className="text-sm text-gray-500 mb-1">Tổng tiền</div>
            <div className="text-2xl font-semibold text-green-700">{formatCurrency(totalAmount)}</div>
          </div>
          
          <div className="bg-red-50 rounded-lg p-4">
            <div className="text-sm text-gray-500 mb-1">Số tiền chưa thanh toán</div>
            <div className="text-2xl font-semibold text-red-700">{formatCurrency(unpaidAmount)}</div>
          </div>
        </div>
      </div>
      ...
      {filteredBills.length === 0 ? (
        <div className="p-6 text-center text-gray-500">
          ...
          <h3 className="mt-2 text-sm font-medium text-gray-900">Không tìm thấy hóa đơn nào</h3>
          ...
          <table className="min-w-full divide-y divide-gray-200">
            ...
            <tbody className="bg-white divide-y divide-gray-200">
              {filteredBills.map((bill) => (
                <tr key={`${bill.loai_hoa_don_goc}-${bill.id_hoa_don}`} className="hover:bg-gray-50">
                  ...
                  <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">{formatDate(bill.ngay_het_han)}</td>
                  <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-900 text-right font-medium">{formatCurrency(bill.tong_tien)}</td>
                  <td className="px-6 py-4 whitespace-nowrap text-center">{formatPaymentStatus(bill.trang_thai)}</td>
                  <td className="px-6 py-4 whitespace-nowrap text-center">
                    <button
                      onClick={() => {/* Thêm xử lý xem chi tiết tại đây */}}
