...
function ResidentRequests() {
  ...
  const filteredRequests = requests.filter((req) => {
    ...
    const searchMatch =
      searchTerm.trim() === "" ||
      (req.ten_dich_vu &&
        req.ten_dich_vu.toLowerCase().includes(searchTerm.toLowerCase())) ||
      (req.tieu_de &&
        req.tieu_de.toLowerCase().includes(searchTerm.toLowerCase())) ||
        ...
        <div className="px-6 py-4 border-b border-gray-200">
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            <div className="bg-blue-50 rounded-lg p-4">
              <div className="text-sm text-gray-500 mb-1">Tổng số yêu cầu</div>
            ...
            <tbody className="bg-white divide-y divide-gray-200">
              {filteredRequests.map((req) => (
                <tr key={req.id_yeu_cau} className="hover:bg-gray-50">
                  ...
                  <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                    {formatDateTime(req.ngay_hen)}
                  </td>
                  <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                    {formatTrangThai(req.trang_thai)}
                  </td>
