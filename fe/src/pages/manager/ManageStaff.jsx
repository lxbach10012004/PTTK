...
function ManageStaff() {
    ...
    const fetchStaff = useCallback(async () => {
        ...
        try {
            ...
            const response = await fetch(`${API_URL}/nguoi-dung?vai_tro=NhanVienPhuTrach`);
            if (!response.ok) {
                 const errData = await response.json().catch(() => ({ error: 'Không thể đọc phản hồi lỗi' }));
                 throw new Error(errData.error || `Lỗi ${response.status}: Không tải được danh sách nhân viên`);
            }
            const data = await response.json();
            // Backend cần trả về thông tin join từ nguoi_dung và nhan_vien_phu_trach
            setStaffList(data);
        } catch (err) {
    ...
    const resetForm = () => {
        ...
        setFormData({ ho_ten: '', email: '', so_dien_thoai: '', mat_khau: '', chuc_vu: '' });
        setMessage('');
        setError('');
    };
    ...
    const handleAddOrUpdate = async (e) => {
        ...
        // Hoặc cần endpoint riêng cho việc tạo/sửa nhân viên
        const url = isEditing ? `${API_URL}/nguoi-dung/staff/${currentStaff.id_nguoi_dung}` : `${API_URL}/nguoi-dung/staff`; // Giả định endpoint này tồn tại
        const method = isEditing ? 'PUT' : 'POST';

        // Prepare data, remove password if editing
        const dataToSend = { ...formData };
        if (isEditing) {
            delete dataToSend.mat_khau; // Don't send password on update via this form
        ...
        try {
            ...
            if (!response.ok) {
                throw new Error(result.error || `Lỗi ${isEditing ? 'cập nhật' : 'thêm'} nhân viên`);
            }
            setMessage(result.message || `Nhân viên đã được ${isEditing ? 'cập nhật' : 'thêm'} thành công.`);
            resetForm();
    ...
    const handleEdit = (staff) => {
        ...
        setFormData({
            ...
            mat_khau: '', // Clear password field for editing
            chuc_vu: staff.chuc_vu || '', // Lấy chức vụ nếu có từ fetchStaff
        });
        setMessage('');
        setError('');
    ...
    const handleDelete = async (staffId) => {
        if (!window.confirm(`Bạn có chắc chắn muốn xóa nhân viên ID ${staffId}? Hành động này không thể hoàn tác.`)) {
            return;
        }
        setMessage('');
        setError('');
        ...
        try {
            ...
            const response = await fetch(`${API_URL}/nguoi-dung/${staffId}`, { method: 'DELETE' }); // Dùng endpoint chung
            const result = await response.json();
            if (!response.ok) {
    ...
    return (
        <div className="bg-white p-6 rounded shadow-md space-y-6">
            ...
            <form onSubmit={handleAddOrUpdate} className="p-4 border rounded bg-gray-50 space-y-3">
                <h2 className="text-lg font-medium">{isEditing ? `Sửa thông tin NV: ${currentStaff?.ho_ten}` : 'Thêm Nhân viên mới'}</h2>
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    ...
                    <div>
                     ...
                     <div>
                        <label htmlFor="chuc_vu" className="block text-sm font-medium text-gray-700">Chức vụ</label>
                        <input type="text" name="chuc_vu" id="chuc_vu" value={formData.chuc_vu} onChange={handleInputChange} className="mt-1 w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm"/>
                    </div>
                    {!isEditing && (
                        <div>
                ...
                <div className="flex items-center space-x-3">
                    <button type="submit" disabled={loading} className="px-4 py-2 bg-purple-600 text-white rounded hover:bg-purple-700 disabled:bg-gray-400">
                        {loading ? 'Đang xử lý...' : (isEditing ? 'Lưu thay đổi' : 'Thêm Nhân viên')}
                    </button>
                    {isEditing && (
            ...
            <div className="overflow-x-auto">
                ...
                {loading && !staffList.length ? <p>Đang tải...</p> : null}
                {!loading && error && !staffList.length ? <p className="text-red-600">{error}</p> : null}
                {!loading && !error && staffList.length === 0 ? <p>Không có nhân viên nào.</p> : (
                    <table className="min-w-full divide-y divide-gray-200">
                        <thead className="bg-gray-50">
                            <tr>
                                ...
                                <th className="px-4 py-2 text-left text-xs font-medium text-gray-500 uppercase">Chức vụ</th> */}
                        ...
                        <tbody className="bg-white divide-y divide-gray-200">
                            {staffList.map(staff => (
                                <tr key={staff.id_nguoi_dung}>
                                    <td className="px-4 py-2 text-sm">{staff.id_nguoi_dung}</td>
                                    <td className="px-4 py-2 text-sm font-medium">{staff.ho_ten}</td>
                                    <td className="px-4 py-2 text-sm">{staff.email}</td>
                                    {/* <td className="px-4 py-2 text-sm">{staff.so_dien_thoai || '-'}</td>
                                    <td className="px-4 py-2 text-sm">{staff.chuc_vu || '-'}</td> Hiển thị chức vụ */}
                                    {/* <td className="px-4 py-2 text-sm">{formatDateTime(staff.ngay_tao)}</td> */} {/* Bỏ nếu không có cột này */}
                                    <td className="px-4 py-2 text-sm space-x-2">
                                        <button onClick={() => handleEdit(staff)} className="text-blue-600 hover:text-blue-800">Sửa</button>
                                        <button onClick={() => handleDelete(staff.id_nguoi_dung)} className="text-red-600 hover:text-red-800">Xóa</button>
                                    </td>
                                </tr>
                            ))}
                        </tbody>
